...
import { activation, mutation } from "../methods"
...
export class Node {
			...
			for (var j = 0; j < nodes.length; j++) {
				...
				if (index > -1) {
					connection.xtrace.values[index] =
						node.connections.self.gain * node.connections.self.weight * connection.xtrace.values[index] +
		...
		if (typeof target.bias !== "undefined") {
			...
			if (target === this) {
				...
				if (this.connections.self.weight !== 0) {
					if (warnings) console.warn("This connection already exists!")
				} else {
	...
	mutate(method) {
		...
		} else if (!(method.name in mutation)) {
	...
	static fromJSON(json) {
		...
		node.type = json.type
		node.mask = json.mask
		node.squash = activation[json.squash]
