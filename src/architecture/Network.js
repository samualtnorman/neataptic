...
import { cost as _cost, mutation as _mutation, rate as _rate } from "../methods"
import { serializeDataSet, workers as _workers } from "../multithreading/multi"
import Neat from "../neat"
import { innovationID } from "./connection"
...
export class Network {
	...
	gate(node, connection) {
		if (this.nodes.indexOf(node) === -1) {
			...
			if (warnings) console.warn("This connection is already gated!")
			return
		}
		...
		switch (method) {
			...
			case mutation.SUB_NODE:
				...
				if (this.nodes.length === this.input + this.output) {
					if (warnings) console.warn("No more nodes left to remove!")
					break
				}
			...
			case mutation.ADD_CONN:
				...
				if (available.length === 0) {
					if (warnings) console.warn("No more connections to be made!")
					break
				}
			...
			case mutation.SUB_CONN:
				...
				if (possible.length === 0) {
					if (warnings) console.warn("No connections to remove!")
					break
				}
			...
			case mutation.MOD_ACTIVATION:
				...
				if (!method.mutateOutput && this.input + this.output === this.nodes.length) {
					if (warnings) console.warn("No nodes that allow mutation of activation function")
					break
				}

				var index = Math.floor(
					Math.random() * (this.nodes.length - (method.mutateOutput ? 0 : this.output) - this.input) +
						this.input
				)
				var node = this.nodes[index]
			...
			case mutation.ADD_SELF_CONN:
				...
				if (possible.length === 0) {
					if (warnings) console.warn("No more self-connections to add!")
					break
				}
			...
			case mutation.SUB_SELF_CONN:
				if (this.selfconns.length === 0) {
					if (warnings) console.warn("No more self-connections to remove!")
					break
				}
			...
			case mutation.ADD_GATE:
				...
				if (possible.length === 0) {
					if (warnings) console.warn("No more connections to gate!")
					break
				}
			...
			case mutation.SUB_GATE:
				...
				if (this.gates.length === 0) {
					if (warnings) console.warn("No more connections to ungate!")
					break
				}
			...
			case mutation.SWAP_NODES:
				...
				if (
					...
					if (warnings) console.warn("No nodes that allow swapping of bias and activation function")
					break
				}
	...
	train(set, options) {
		...
		if (typeof options.rate === "undefined") {
			if (warnings) console.warn("Using default learning rate, please define a rate!")
		}
		if (typeof options.iterations === "undefined") {
			if (warnings) console.warn("No target iterations given, running until error is reached!")
		}

		// Read the options
		var targetError = options.error || 0.05
		var cost = options.cost || _cost.MSE
		var baseRate = options.rate || 0.3
		var dropout = options.dropout || 0
		var momentum = options.momentum || 0
		var batchSize = options.batchSize || 1 // online learning
		var ratePolicy = options.ratePolicy || _rate.FIXED()
	...
	async evolve(set, options) {
		...
		if (threads === 1) {
			...
			var converted = serializeDataSet(set)

			// Create workers, send datasets
			var workers = []
			if (typeof window === "undefined") {
				for (var i = 0; i < threads; i++) {
					workers.push(new _workers.node.TestWorker(converted, cost))
				}
			} else {
				for (var i = 0; i < threads; i++) {
					workers.push(new _workers.browser.TestWorker(converted, cost))
				}
			}
	...
	static crossOver(network1, network2, equal) {
		...
		for (i = 0; i < network1.connections.length; i++) {
			...
			n1conns[innovationID(data.from, data.to)] = data
		}
		...
		for (i = 0; i < network2.connections.length; i++) {
			...
			n2conns[innovationID(data.from, data.to)] = data
		}
