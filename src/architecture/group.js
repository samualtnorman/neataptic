...
import { connection as _connection, gating } from "../methods"
	...
	connect(target, method, weight) {
		...
		if (target instanceof Group) {
			if (typeof method === "undefined") {
				if (this !== target) {
					if (warnings) console.warn("No group connection specified, using ALL_TO_ALL")
					method = _connection.ALL_TO_ALL
				} else {
					if (warnings) console.warn("No group connection specified, using ONE_TO_ONE")
					method = _connection.ONE_TO_ONE
				}
			}
			if (method === _connection.ALL_TO_ALL || method === _connection.ALL_TO_ELSE) {
				for (i = 0; i < this.nodes.length; i++) {
					for (j = 0; j < target.nodes.length; j++) {
						if (method === _connection.ALL_TO_ELSE && this.nodes[i] === target.nodes[j]) continue
			...
			} else if (method === _connection.ONE_TO_ONE) {
