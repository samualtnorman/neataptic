...
import { crossover, mutation, selection } from "./methods"
...
export class Neat {
	...
	selectMutationMethod(genome) {
		...
		if (mutationMethod === mutation.ADD_NODE && genome.nodes.length >= this.maxNodes) {
			if (warnings) console.warn("maxNodes exceeded!")
			return
		}

		if (mutationMethod === mutation.ADD_CONN && genome.connections.length >= this.maxConns) {
			if (warnings) console.warn("maxConns exceeded!")
			return
		}

		if (mutationMethod === mutation.ADD_GATE && genome.gates.length >= this.maxGates) {
			if (warnings) console.warn("maxGates exceeded!")
			return
		}
