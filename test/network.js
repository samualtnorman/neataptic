...
import { architect, Network, methods, config } from "../dist/neataptic.js"
...
describe("Networks", function() {
	...
	describe("Learning capability", function() {
		...
		it("NOT gate", function() {
			learnSet([ { input: [ 0 ], output: [ 1 ] }, { input: [ 1 ], output: [ 0 ] } ], 1000, 0.002)
		})
		...
		it("SIN + COS", function() {
			...
			while (set.length < 100) {
				...
				set.push({
					input: [ inputValue / (Math.PI * 2) ],
					output: [ (Math.sin(inputValue) + 1) / 2, (Math.cos(inputValue) + 1) / 2 ]
				})
			}
